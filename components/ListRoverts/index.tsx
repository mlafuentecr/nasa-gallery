...
const ListRoverts = () => {
	...
	return (
		<Listbox value={searchContext?.rover} onChange={e => searchContext?.setRover(e.name)}>
			{({ open }) => (
				<div className='relative mt-1 z-30'>
					<Listbox.Button className='border-gray-800 flex justify-between bg-slate-700 text-white relative w-full cursor-default rounded-lg  py-2 px-3  text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm'>
						<span className='block truncate'>{searchContext?.rover ? searchContext?.rover : 'Select an item'}</span>
						<span>▼</span>
					</Listbox.Button>
					{open && (
							...
							<Listbox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
								{projects.map(item => (
									<Listbox.Option key={item.id} value={item}>
										{({ active, selected }) => (
											<div className={`${active ? 'bg-amber-100 text-amber-900' : 'text-gray-900'} cursor-pointer select-none py-2 pl-10 pr-4`}>
												<span className={`block truncate ${searchContext?.rover ? 'font-medium' : 'font-normal'}`}>{item.name}</span>
											</div>
										)}
									</Listbox.Option>
								))}
